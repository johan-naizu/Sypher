...
class Fun(commands.Cog):
    ...
    @commands.hybrid_command(description='Rolls a specified number of dice', usage='roll [limit]')
    async def roll(self,ctx:commands.Context, n: int =1):
        await ctx.defer()
        if type(n) == str:
    ...
    @commands.hybrid_command(description='Tosses a specified number of coins', usage='toss [limit]')
    async def toss(self,ctx:commands.Context, n:int =1):
        await ctx.defer()
        if type(n) == str:
    ...
    @commands.hybrid_command(description='Gives a random topic to start a conversation', usage='topic')
    async def topic(self, ctx:commands.Context):
        await ctx.defer()
        topic=await utils.get_topic()
        await ctx.send(f"**{topic}**")

    @commands.hybrid_command(description='Get definitions from Urban Dictionary', usage='urban {word}')
    @commands.is_nsfw()
    ...
    @commands.hybrid_command(description='Gives random memes', usage='meme')
    ...
    @commands.hybrid_command(description='Roast a user', usage='roast [user]')
    @commands.is_nsfw()
